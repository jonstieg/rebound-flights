...
class ReBound():
    def reboundSearch(self, dateGo, departingAirport, arrivingAirport, departingWeekday, returningWeekday, maxStopsDeparting, maxStopsReturning, departingTimeEarly, departingTimeEarlyAMPM, departingTimeLate, departingTimeLateAMPM, returningTimeEarly, returningTimeEarlyAMPM, returningTimeLate, returningTimeLateAMPM):
        ...
        results = ["Results:"]
        for x in range(1, 20):
            results.append("On {:%b, %d %Y} your flight will cost $".format(theDate) + str(randint(192,877)))
            theDate += timedelta(7)
        results.append("Unfortunately, Google deprecated their search API so all of the above prices are dummy data but this is how the site used to work.")
        return results
    ...
    #     departingTimeEarly = "00:00" if departingTimeEarly == '' else str(
    #         int(departingTimeEarly) + int(departingTimeEarlyAMPM)) + ":00"
    #     returningTimeEarly = "00:00" if returningTimeEarly == '' else str(
    #         int(returningTimeEarly) + int(returningTimeEarlyAMPM)) + ":00"
    #     departingTimeLate = "23:59" if departingTimeLate == '' else str(
    #         int(departingTimeLate) + int(departingTimeLateAMPM)) + ":00"
    #     returningTimeLate = "23:59" if returningTimeLate == '' else str(
    #         int(returningTimeLate) + int(returningTimeLateAMPM)) + ":00"
    #     weeksToSearch = 1


    #     api_key = os.environ.get('googAPI')
    ...
    #     for x in range(1, weeksToSearch+1):
    #         try:
    #             flights[data[x]["trips"]["tripOption"][0]["slice"][0]["segment"][0]["leg"]
    #                     [0]["departureTime"]] = data[x]["trips"]["tripOption"][0]["saleTotal"][3:]
    #         except:
    #             pass
